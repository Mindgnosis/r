...
# you need to install two R libraries if you don't have them already: VGAM and igraph
# Vinh Modified as of 10/19/13
...
#g = read.graph("/Users/ladamic/Downloads/wikipedia/wikipedia.gml",format="gml")

g = read.graph("G:/scorpion/professional/sandbox/snaUofM/data/wikipedia.gml",format="gml")
...
V(g)$label[which.max( inDegrees )]
V(g)$label[which.max( outDegrees )]
...
unDirBetw = betweenness.estimate( g, directed = "FALSE", cutoff = 0 )
maxBetwnIndex = which.max( unDirBetw )

# experimenting with listing out attributes ###
hiBetw_Node = V(g)[ maxBetwnIndex ]
names( attributes( hiBetw_Node ) )
str( names( attributes( hiBetw_Node ) ) )

# experimenting with referencing Vertices
hiBetw_nLbl = V(g)$label[ maxBetwnIndex ]
...
# How to V: Vertex http://igraph.sourceforge.net/doc/R/iterators.html
#V(g)$label[V(g)[nei( which.max( unDirBetw ) )]]
V(g)$label[V(g)[nei( maxBetwnIndex )]]
...
pgRank = page.rank( g, directed = "TRUE", damping = 0.85 )$vector
V(g)$label[ which.max( pgRank ) ]

## Results ################################
## Power-law exponent: a$alpha = 2.67
